...
#
#
# @patch('fast_agave.middlewares.loggers.LOGS_SERVER_URL', True)
# def test_log_post_data_to_opensearch(client: TestClient) -> None:
#     data = dict(name='Doroteo Arango')
#     auth = auth_header('AK123', 'secret')
#     auth['X-Cuenca-Token'] = 'TK123456789'
#     auth['X-Cuenca-LoginToken'] = 'LT123456789'
#     auth['X-Cuenca-LoginId'] = 'LI123456789'
#     auth['X-Cuenca-SessionId'] = 'SS123456789'
#
#     resp = client.post('/accounts', json=data, headers=auth)
...
#
# @patch('fast_agave.middlewares.loggers.LOGS_SERVER_URL', True)
# def test_log_get_data_to_opensearch(
...
#     auth['X-Cuenca-SessionId'] = 'SS123456789'
#
#     resp = client.get(f'/accounts/{account.id}', headers=auth)
#     assert resp.status_code == 200
#
#
# @patch('fast_agave.middlewares.loggers.LOGS_SERVER_URL', True)
# def test_log_query_string_to_opensearch(
#     client: TestClient, card: Card
# ) -> None:
#     auth = auth_header('AK123', 'secret')
#     auth['X-Cuenca-Token'] = 'TK123456789'
#     auth['X-Cuenca-LoginToken'] = 'LT123456789'
...
#     auth['X-Cuenca-SessionId'] = 'SS123456789'
#
#     resp = client.get(f'/cards?number={card.number}', headers=auth)
#     assert resp.status_code == 200
